   ...
   public function index()
{
    ...
    $upcomingHotels = $hotelReservations->filter(function ($res) use ($today) {
        ...
        return in_array($res->status_id, [1, 3]) && \Carbon\Carbon::parse($res->end_at)->startOfDay() >= $today;
    });
    ...
    $upcomingRestaurants = $restaurantReservations->filter(function ($res) use ($today) {
        return in_array($res->status_id, [1, 3]) && \Carbon\Carbon::parse($res->start_at)->startOfDay() >= $today;
    });
