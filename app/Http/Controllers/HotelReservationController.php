...
class HotelReservationController extends Controller
{
    ...
    public function show($hotelId, Request $request)
    {
        ...
        $hotel = Hotel::with([
            'roomTypes' => function ($query) use ($request) {
                // detail-hotelで選択されたルームタイプIDがあれば絞り込む
                if ($request->has('room_type_id')) {
                    $query->where('type_id', $request->room_type_id);
                }
                $query->with('roomType');
            },
            'images'
    ...
    public function cancelConfirm(string $reservationId)
    {
        ...
        // すでにキャンセル済みなら一覧に戻す
        $canceledId = DB::table('statuses')->where('name', 'Cancelled')->value('id') ?? 5;
    ...
    public function cancel(Request $request, string $reservationId)
    {
        ...
        // statuses テーブルから 'Cancelled'(id=5) を取得
        $canceledId = DB::table('statuses')->where('name', 'Cancelled')->value('id') ?? 5;
