                                    ...
                                    <div class="modal-dialog modal-lg modal-dialog-centered">
                                        <div class="modal-content border-0 rounded-4 overflow-hidden">
                                                ...
                                                <div class="row">
                                                        ...
                                                        <div class="room-images hide-scrollbar">
                                                            @foreach ($room->images as $image)
                                                                <img src="{{ asset('storage/' . $image->image) }}"
                                                                    class="w-100 rounded-3" alt="Room Image"
                                                    ...
                                                    <div class="col-md-6">
                                                        <div class="d-flex justify-content-between align-items-start mb-2">
                                                            ...
                                                            <span
                                                                class="badge {{ $isAvailable ? 'bg-success' : 'bg-danger' }} px-3">{{ $room->status->name }}</span>
                                                        </div>
                                                        <p class="text-muted small">Room No.
                                                            {{ $room->room_number ?? 'N/A' }}</p>
                                                        <h6 class="text-uppercase text-muted small fw-bold mt-4">Description
                                                        </h6>
                                            ...
                                            <div class="modal-footer border-0 p-4 pt-0">
                                                ...
                                                @if ($isAvailable)
                                                    <a href="{{ route('user.hotels.reservation') }}"
                                                        class="btn btn-primary rounded-pill px-5 fw-bold shadow">Reserve
                                                        Now</a>
                                                @endif
                                            </div>
                                        </div>
                                    ...
                                    <div
                                        ...
                                        <div class="row g-0">
                                            ...
                                            <div class="col-md-5 bg-light position-relative border-end">
                                                <div class="room-images hide-scrollbar"
                                                    style="{{ !$isAvailable ? 'filter: grayscale(80%);' : '' }}">
                                                    @foreach ($room->images as $image)
                                                        <img src="{{ asset('storage/' . $image->image) }}"
                                                            alt="room image">
                                                    @endforeach
                                                ...
                                                <div class="p-3 bg-white">
                                                    <h5 class="fw-bold mb-2">{{ $room->type->name ?? 'Standard Room' }}
                                                    </h5>
                                                    <div class="d-flex gap-2">
                                                        <span class="badge bg-light text-dark border fw-normal px-2 py-1">
                                                            <i class="fa-solid fa-stairs me-1 text-primary"></i> Floor
                                                            {{ $room->floor_number }}
                                                        </span>
                                                        <span class="badge bg-light text-dark border fw-normal px-2 py-1">
                                                            <i class="fa-solid fa-user me-1 text-primary"></i> Max
                                                            {{ $room->max_guests }}
                                                        </span>
                                                    </div>
                                                </div>
                                            ...
                                            {{-- Room Details (Right) --}}
                                            <div class="col-md-7 d-flex flex-column" style="cursor: pointer;"
                                                data-bs-toggle="modal" data-bs-target="#roomModal{{ $room->id }}">
                                                <div class="card-body p-4 d-flex flex-column h-100">
                                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                                        <small class="text-uppercase text-muted fw-bold">Room
                                                            Description</small>
                                                    ...
                                                    <div class="row align-items-center mt-3 border-top pt-3">
                                                        <div class="col-6">
                                                            ...
                                                            <span
                                                                class="h3 fw-bold {{ $isAvailable ? 'text-primary' : 'text-muted' }} mb-0">
                                                        ...
                                                        <div class="col-6 text-end">
                                                            @if ($isAvailable)
                                                                <a href="#"
                                                                    class="btn btn-primary btn-lg px-4 rounded-pill fw-bold shadow-sm">
                                                                    Book Now
                                                                </a>
                                                            @else
                                                                <button
                                                                    class="btn btn-secondary btn-lg px-4 rounded-pill disabled shadow-none">Unavailable</button>
                                                            @endif
                                                        </div>
                                                    </div>
                                                </div>
                                    ...
                                    <div class="modal fade" id="roomModal{{ $room->id }}" tabindex="-1"
                                        ...
                                        <div class="modal-dialog modal-lg modal-dialog-centered">
                                            <div class="modal-content border-0 rounded-4 overflow-hidden">
                                                ...
                                                <div class="modal-body p-4">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-6">
                                                            ...
                                                            <p class="text-muted small">Room No.
                                                                {{ $room->room_number ?? 'N/A' }}</p>
                                                            <h6 class="text-uppercase text-muted small fw-bold mt-4">
                                                                Description</h6>
                                                            <p class="text-dark small lh-lg">{{ $room->detail }}</p>
                                                ...
                                                {{-- 詳細画面 (hotels/show.blade.php) のモーダル内 --}}
                                                <div class="modal-footer border-0 p-4 pt-0">
                                                    <button type="button" class="btn btn-light rounded-pill px-4"
                                                        data-bs-dismiss="modal">Close</button>

                                                    @if ($isAvailable)
                                                        {{-- 送信先を「ルームタイプ一覧」を表示するメソッドに変更 --}}
                                                        <form method="GET"
                                                            action="{{ route('user.hotels.show', ['hotel' => $hotel->id]) }}">
                                                            {{-- セッションをリセットしたい場合はこのフラグを残す --}}
                                                            <input type="hidden" name="clear_reservation_session"
                                                                value="1">
                                                            ...
                                                            <button type="submit"
                                                                class="btn btn-primary rounded-pill px-5 fw-bold shadow">
                                                                Reserve Now
                                                            </button>
                                                        </form>
                                                    @endif
